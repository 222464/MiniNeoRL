...
class LayerRL:
    ...
    def learn(self, reinforce, targetExp, feedBackPrev, learnEncoderRate, learnDecoderRate, learnBiasRate, traceDecay):
        ...
        self._predictiveTraces = np.multiply(np.repeat(1.0 - self._statesPrev.T, len(self._input), 0), self._predictiveTraces) * traceDecay + np.dot(predErrorExp, self._statesPrev.T)
        self._feedBackTraces = np.multiply(np.repeat(1.0 - feedBackPrev.T, len(self._input), 0), self._feedBackTraces) * traceDecay + np.dot(predErrorExp, feedBackPrev.T)
