...
class Hierarchy:
        ...
        for l in range(0, len(layerSizes)):
            ...
            if l == 0:
                if l < len(layerSizes) - 1:
                    layer = Layer(numInputs, layerSizes[l], layerSizes[l + 1], initMinWeight, initMaxWeight, activeRatio)
                else:
    ...
    def simStep(self, input, learnEncoderRate, learnDecoderRate, learnBiasRate, traceDecay):
        # Up pass
        ...
        for l in range(0, len(self._layers)):
            if l == 0:
                if l < len(self._layers) - 1:
                    self._layers[l].learn(input, self._layers[l + 1]._predictionsPrev, learnEncoderRate, learnDecoderRate, learnBiasRate, traceDecay)
                else:
                    self._layers[l].learn(input, np.matrix([[ 0 ]]), learnEncoderRate, learnDecoderRate, learnBiasRate, traceDecay)
            else:
                if l < len(self._layers) - 1:
